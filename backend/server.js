...
const { v4: uuidv4 } = require("uuid"); // Добавляем uuid для генерации qr_code
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (["teacher", "parent", "school_admin"].includes(role) && !name) {
        return res.status(400).json({ error: "Name is required for teacher, parent, or school_admin roles" });
      }
...
app.get(
  "/schools/:id",
  authMiddleware,
  roleMiddleware(["main_admin", "school_admin", "teacher", "parent"]),
  async (req, res) => {
    try {
...
app.post(
  ...
  async (req, res) => {
    try {
      const { name, group, specialty, email, password, enrolled_events = [], qr_code: providedQrCode } = req.body;
...
app.post(
  ...
  async (req, res) => {
    try {
      const { name, schedule, description, is_active } = req.body;
    ...
    try {
      const query =
        req.user.role === "school_admin"
          ...
          : { _id: req.params.id };
      const { name, schedule, description, is_active } = req.body;
      const updates = { name, schedule, description, is_active, updated_at: new Date() };
      const event = await Event.findOneAndUpdate(query, updates, { new: true });
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      if (!event) return res.status(404).json({ error: "Event not found" });
      // Если деактивируем мероприятие, удаляем все записи посещаемости
      if (is_active === false) {
        await AttendanceRecord.deleteMany({ event_name: event.name });
        console.log(`All attendance records for event ${event.name} deleted`);
      }
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      if (!event) return res.status(404).json({ error: "Event not found" });
      // Удаляем все записи посещаемости для этого мероприятия
      await AttendanceRecord.deleteMany({ event_name: event.name });
...
app.get(
  ...
  authMiddleware,
  roleMiddleware(["school_admin", "main_admin", "parent"]),
  async (req, res) => {
    try {
